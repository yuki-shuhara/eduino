...
public class SegPanel extends PanelTranslate{
  ...
  private void setOutLine(){
    ...
    //
     ...
     outLine.reset();
     outLine.addPoint(this.x, this.y+polygon.ypoints[5]);
     for(int i=6; i<9; i++){
...
//    g2.setStroke(bs);
  ...
  public void setPanelTranslate(int x, int y, PanelTranslate p) {
    if(super.nextSetis){
      ...
      if(setPosition != null){
        if(setPosition == firstPanel && firstPanel != null) return; //1個しか間に入れないようにするため
        setPosition = p;
